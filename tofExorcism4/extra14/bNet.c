...
//#ifndef lint
//static char rcsid[] UTIL_UNUSED = "$Id: bnet.c,v 1.18 1998/09/13 23:06:00 fabio Exp fabio $";
//#endif

static char BuffLine[MAXLENGTH];
char *CurPos;					/* I used this variable outside  -alanmi */
...
char **readList ARGS( ( FILE * fp, int *n ) );	/* I used this function outside  -alanmi */
static void printList ARGS( ( char **list, int n ) );
static char **bnetGenerateNewNames ARGS( ( st_table * hash, int n ) );
static int bnetDumpReencodingLogic
ARGS( ( DdManager * dd, char *mname, int noutputs, DdNode ** outputs,
		char **inames, char **altnames, char **onames, FILE * fp ) );
#if 0
static int bnetBlifXnorTable ARGS( ( FILE * fp, int n ) );
#endif
static int bnetBlifWriteReencode
ARGS( ( DdManager * dd, char *mname, char **inames, char **altnames,
		int *support, FILE * fp ) );
static int *bnetFindVectorSupport
ARGS( ( DdManager * dd, DdNode ** list, int n ) );
static int buildExorBDD
ARGS( ( DdManager * dd, BnetNode * nd, st_table * hash, int params,
		int nodrop ) );
...
static int bnetDfsOrder
ARGS( ( DdManager * dd, BnetNetwork * net, BnetNode * node ) );
...
/* -------------------------------*/
/* fixed by alanmi Jan 08, 2001 */
			strcmp( savestring, ".area" ) == 0 ||
			strcmp( savestring, ".delay" ) == 0 ||
			strcmp( savestring, ".wire" ) == 0 ||
			strcmp( savestring, ".input_arrival" ) == 0 ||
			strcmp( savestring, ".default_input_arrival" ) == 0 ||
			strcmp( savestring, ".output_required" ) == 0 ||
			strcmp( savestring, ".default_output_required" ) == 0 ||
			strcmp( savestring, ".input_drive" ) == 0 ||
			strcmp( savestring, ".default_input_drive" ) == 0 ||
			strcmp( savestring, ".max_input_load" ) == 0 ||
			strcmp( savestring, ".default_max_input_load" ) == 0 ||
			strcmp( savestring, ".output_load" ) == 0 ||
			strcmp( savestring, ".default_output_load" ) == 0 ||
/* -------------------------------*/
			strcmp( savestring, ".names" ) == 0
			|| strcmp( savestring, ".end" ) == 0 )
	{
/*----------------------------------*/
		if ( strcmp( savestring, ".area" ) == 0 ||
			 strcmp( savestring, ".delay" ) == 0 ||
			 strcmp( savestring, ".wire" ) == 0 ||
			 strcmp( savestring, ".input_arrival" ) == 0 ||
			 strcmp( savestring, ".default_input_arrival" ) == 0 ||
			 ...
			 strcmp( savestring, ".default_output_load" ) == 0 )
		{
			...
			if ( n < 1 )
			{
				( void ) fprintf( stdout, "Empty list.\n" );
				goto failure;
			}
			for ( i = 0; i < n; i++ )
				FREE( list[i] );
...
/*----------------------------------*/
	...
	while ( newnode != NULL )
	{
		/* insert the node into the hash table only if it is not an EXDC node */
		/* fixed by Alanmi January 16, 2001 */
		if ( !newnode->exdc_flag || newnode->type != BNET_OUTPUT_NODE )
			/* ------------------------------------------------------------------ */
		{
			if ( pr > 2 )
				printf( "Inserting %s\n", newnode->name );
	...
	while ( newnode != NULL )
	{
		...
		{
			if ( !st_lookup
				 ( net->hash, newnode->inputs[i], ( char ** ) &auxnd ) )
			{
...
/* This is a dirty hack: 
   Originally, function Bnet_BuildNodeBDD() assigned a new variable each 
   time it was called. As a result, it was impossible to read two networks 
   using the same bdd manager, because the variables for the second networks 
   continued the range started by the variables of the first network...
   Here the counter of variables is introduced. This counter should be set 
   to zero each time before the new network is processed.    -alanmi, Sep 24,2000
*/

int Bnet_CounterOfBddVariables;

void
Bnet_ResetCounterOfBddVariables(  )
{
	Bnet_CounterOfBddVariables = 0;
}

/******************************************************************************/
int
Bnet_BuildNodeBDD( DdManager * dd /* DD manager */ ,
...
/*			func = Cudd_bddNewVar(dd);                                      */
			func = Cudd_bddIthVar( dd, Bnet_CounterOfBddVariables++ );
			if ( func == NULL )
				goto failure;
			nd->var = func->index;
			nd->active = TRUE;
		...
		while ( line != NULL )
		{
			...
			for ( i = 0; i < nd->ninp; i++ )
			{
				if ( line->values[i] == '-' )
					continue;
				if ( !st_lookup
					 ( hash, ( char * ) nd->inputs[i], ( char ** ) &auxnd ) )
					goto failure;
				...
				{				/* params == BNET_GLOBAL_DD */
					if ( auxnd->dd == NULL )
					{
						if ( !Bnet_BuildNodeBDD
							 ( dd, auxnd, hash, params, nodrop ) )
							goto failure;
					}
					if ( line->values[i] == '1' )
						var = auxnd->dd;
...
******************************************************************************/
int
Bnet_DfsVariableOrder( DdManager * dd, BnetNetwork * net )
{
	BnetNode **roots;
	BnetNode *node;
	int nroots;
...
******************************************************************************/
int
Bnet_bddDump( DdManager * dd /* DD manager */ ,
			  BnetNetwork *
			  ...
			  int reencoded /* whether variables have been reencoded */  )
{
	...
	for ( i = 0; i < network->nlatches; i++ )
	{
		...
		if ( !st_lookup
			 ( network->hash, network->latches[i][0], ( char ** ) &node ) )
		{
			goto endgame;
		}
		outputs[i] = node->dd;
	...
	for ( i = 0; i < network->npos; i++ )
	{
		...
		if ( !st_lookup
			 ( network->hash, network->outputs[i], ( char ** ) &node ) )
		{
			goto endgame;
		}
	...
	{
		if ( !st_lookup
			 ( network->hash, network->inputs[i], ( char ** ) &node ) )
		{
			goto endgame;
		}
	...
	{
		if ( !st_lookup
			 ( network->hash, network->latches[i][1], ( char ** ) &node ) )
		{
			goto endgame;
		}
	...
	if ( reencoded == 1 && dumpFmt == 1 )
	{
		...
		retval =
			bnetDumpReencodingLogic( dd, network->name, noutputs, outputs,
	...
	{
		retval =
			Cudd_DumpDaVinci( dd, noutputs, outputs, inames, onames, dfp );
	}
	else if ( dumpFmt == 3 )
	{
	...
	{
		retval =
			Cudd_DumpFactoredForm( dd, noutputs, outputs, inames, onames,
								   dfp );
	}
	else
	{
...
******************************************************************************/
int
Bnet_bddArrayDump( DdManager * dd /* DD manager */ ,
				   BnetNetwork *
				   network /* network whose BDDs should be dumped */ ,
				   char *dfile /* file name */ ,
...
*****************************************************************************/
int
Bnet_ReadOrder( DdManager * dd,
				char *ordFile, BnetNetwork * net, int locGlob, int nodrop )
{
	FILE *fp;
	st_table *dict;
	int result;
	...
	while ( !feof( fp ) )
	{
		...
		if ( !st_lookup( net->hash, name, ( char ** ) &node ) )
		{
...
*****************************************************************************/
int
Bnet_PrintOrder( BnetNetwork * net, DdManager * dd )
{
...
/**Function********************************************************************
  ...
  [ The statement about only two pieces of memory does not seem to be true  -alanmi ]
		  ...
		  int *n /* on return, number of strings in the list */  )
{
	...
	while ( CurPos )
	{
		...
		/*--------------------------------------------------------------------*/
		/* in some cases, the continuation character (\) is attached to the last
		   ** input name on the line without the space separator - here we check for this 
		   **                                         -alanmi, September 21, 2000
		 */
		while ( CurPos[length - 1] == '\\' )
		{
			/* remove the continuator */
			CurPos[length - 1] = '\0';
...
******************************************************************************/
static char **
bnetGenerateNewNames( st_table *
					  hash /* table of existing names (or NULL) */ ,
...
******************************************************************************/
static int
bnetDumpReencodingLogic( DdManager * dd /* DD manager */ ,
						 ...
						 FILE * fp /* file pointer */  )
{
	...
	retval =
		bnetBlifWriteReencode( dd, mname, inames, altnames, support, fp );
	if ( retval == EOF )
		goto failure;
...
#if 0
static int
bnetBlifXnorTable( FILE * fp /* file pointer */ ,
...
******************************************************************************/
static int
bnetBlifWriteReencode( DdManager * dd,
					   char *mname,
					   char **inames,
					   char **altnames, int *support, FILE * fp )
{
	int retval;
	int nvars = Cudd_ReadSize( dd );
	int i, j;
...
******************************************************************************/
static int *
bnetFindVectorSupport( DdManager * dd, DdNode ** list, int n )
{
	DdNode *support = NULL;
	DdNode *scan;
	int *array = NULL;
...
******************************************************************************/
static int
buildExorBDD( DdManager * dd,
			  BnetNode * nd, st_table * hash, int params, int nodrop )
{
	int check[8];
	int i;
	int nlines;
			 ...
			 BnetNode * nd, st_table * hash, int params, int nodrop )
{
	...
	/* Get the inputs. */
	if ( !st_lookup
		 ( hash, ( char * ) nd->inputs[controlC], ( char ** ) &auxnd ) )
	{
		goto failure;
	}
	if ( params == BNET_LOCAL_DD )
	...
	if ( !st_lookup
		 ( hash, ( char * ) nd->inputs[mux[0]], ( char ** ) &auxnd ) )
	{
		goto failure;
	}
	if ( params == BNET_LOCAL_DD )
	...
	if ( !st_lookup
		 ( hash, ( char * ) nd->inputs[mux[1]], ( char ** ) &auxnd ) )
	{
		goto failure;
	}
	if ( params == BNET_LOCAL_DD )
...
bnetSetLevel( BnetNetwork * net )
{
	...
	/* Recursively visit nodes. This is pretty inefficient, because we
	   ...
	   ** all nodes will be reached even if there are dangling outputs. */
	node = net->nodes;
	while ( node != NULL )
	{
...
******************************************************************************/
static BnetNode **
bnetOrderRoots( BnetNetwork * net, int *nroots )
{
	int i, noutputs;
	BnetNode *node;
	BnetNode **nodes = NULL;
...
******************************************************************************/
static int
bnetLevelCompare( BnetNode ** x, BnetNode ** y )
{
...
******************************************************************************/
static int
bnetDfsOrder( DdManager * dd, BnetNetwork * net, BnetNode * node )
{
	int i;
	BnetNode *auxnd;
	BnetNode **fanins;
