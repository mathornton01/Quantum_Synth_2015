/**CFile***********************************************************************
  ...
  Synopsis    [Procedures to compute the symmetry information.]

  Description [External procedures included in this module:
				<ul>
				<li> Extra_SymmPairsComputeFast();
				<li> Extra_SymmPairsComputeSlow();
				<li> Extra_SymmPairsComputeNaive();
				<li> Extra_bddCheckVarsSymmetricNaive();
				<li> Extra_SymmPairsAllocate();
				<li> Extra_SymmPairsDissolve();
				<li> Extra_SymmPairsPrint();
				<li> Extra_SymmPairsCreateFromZdd();
				<li> Extra_zddSymmPairsCompute();
				<li> Extra_zddGetSymmetricVars();
				<li> Extra_zddGetSingletons();
				<li> Extra_bddReduceVarSet();
				<li> Extra_bddCheckVarsSymmetric();
				</ul>
			Internal procedures included in this module:
				<ul>
				<li> extraZddSymmPairsCompute();
				<li> extraZddGetSymmetricVars();
				<li> extraZddGetSingletons();
				<li> extraBddReduceVarSet();
				<li> extraBddCheckVarsSymmetric();
				</ul>
			Static procedures included in this module:
				<ul>
				<li> extraBddCountMintermsCofactor();
				<li> extraBddCountMinterms();
				</ul>
	      ]
  ...
  Revision    [$bSymm.c, v.1.2, November 15, 2001, alanmi $]
...
static double Extra_bddCountMintermsCofactor( DdManager * dd, DdNode * bFunc, DdNode * bVar );
static double  extraBddCountMintermsCofactor( DdManager * dd, DdNode * bFunc, DdNode * bVar );
static double Extra_bddCountMinterms( DdManager * dd, DdNode * bFunc );
static double  extraBddCountMinterms( DdManager * dd, DdNode * bFunc );
...
/**Function********************************************************************
  ...
  Description [Returns the symmetry information in the form of symminfo structure.]
...
******************************************************************************/
symminfo * Extra_SymmPairsComputeFast( 
...
} /* end of Extra_SymmPairsComputeFast */
...
******************************************************************************/
symminfo * Extra_SymmPairsAllocate( int nVars )
{
	int i;
	symminfo * p;
...
******************************************************************************/
void Extra_SymmPairsDissolve( symminfo * p )
{
	free( p->pVars );
	free( p->pSymms[0] );
	free( p->pSymms    );
...
******************************************************************************/
void Extra_SymmPairsPrint( symminfo * p )
{
	int i, k, m;
...
******************************************************************************/
symminfo * Extra_SymmPairsCreateFromZdd( DdManager * dd, DdNode * zPairs, DdNode * bSupp )
{
	int i;
	int nSuppSize;
	symminfo * p;
...
/**Function********************************************************************
  ...
  Description [Uses the algorithm proposed in D.Moller et al. "Detection of 
  Symmetry of Boolean Functions Represented by ROBDDs". ICCADÆ93.]
...
******************************************************************************/
symminfo * Extra_SymmPairsComputeSlow( DdManager * dd, DdNode * bFunc )
{
	DdNode * bSupp, * bTemp;
	int nSuppSize;
	symminfo * p;
	...
	MintCofs[0] = (double*) malloc( nSuppSize * sizeof(double) );
	MintCofs[1] = (double*) malloc( nSuppSize * sizeof(double) );

	// fill in the cofactor number of minterms
	clk = clock();
	for ( i = 0, bTemp = bSupp; bTemp != b1; bTemp = cuddT(bTemp), i++ )
	{
		MintCofs[0][i] = Extra_bddCountMintermsCofactor( dd, bFunc, Cudd_Not(dd->vars[bTemp->index]) );
		MintCofs[1][i] = Extra_bddCountMintermsCofactor( dd, bFunc,          dd->vars[bTemp->index]  );
//		printf( "MintCofs[0][%d] = %d\n", i, (int)MintCofs[0][i] );
//		printf( "MintCofs[1][%d] = %d\n", i, (int)MintCofs[1][i] );
	}
//	printf( "Minterm computation time = %.2f sec\n", (float)(clock()-clk)/(float)(CLOCKS_PER_SEC) );

	// go through the candidate pairs and check using Idea1
	for ( i = 0; i < nSuppSize; i++ )
	for ( k = i+1; k < nSuppSize; k++ )
		p->pSymms[i][k] = (int)( MintCofs[0][i] == MintCofs[0][k] && MintCofs[1][i] == MintCofs[1][k] );

	// delocate the minterm info
	free( MintCofs[0] );
	free( MintCofs[1] );

	// go through the candidate pairs and check using Idea2
	for ( i = 0; i < nSuppSize; i++ )
	for ( k = i+1; k < nSuppSize; k++ )
		if ( p->pSymms[i][k] )
			 p->pSymms[i][k] = Extra_bddCheckVarsSymmetric( dd, bFunc, p->pVars[i], p->pVars[k] );

	// go through the remaining and check them using the naive method
	for ( i = 0; i < nSuppSize; i++ )
	for ( k = i+1; k < nSuppSize; k++ )
	{
		if ( p->pSymms[i][k] )
...
} /* end of Extra_SymmPairsComputeSlow */
...
******************************************************************************/
symminfo * Extra_SymmPairsComputeNaive( DdManager * dd, DdNode * bFunc )
{
	DdNode * bSupp, * bTemp;
	int nSuppSize;
	symminfo * p;
...
/**Function********************************************************************
  ...
  Synopsis    [Counts the number of minterms in the cofactor of the function w.r.t. a variable.]
...
******************************************************************************/
double Extra_bddCountMintermsCofactor( DdManager * dd, DdNode * bFunc, DdNode * bVar )
{
/*
	DdNode * bCof;
	double Res;
	bCof = Cudd_Cofactor( dd, bFunc, bVar ); Cudd_Ref( bCof );
	Res = Cudd_CountMinterm( dd, bCof, dd->size );
	Cudd_RecursiveDeref( dd, bCof );
	return Res;
*/
	assert( dd->size < 1024 );
	if ( s_Max == 0.0 )
		s_Max = Extra_Power2( dd->size );
	return extraBddCountMintermsCofactor( dd, bFunc, bVar );
} /* end of Extra_bddCountMintermsCofactor */

/**Function********************************************************************

  Synopsis    [Counts the number of minterms in the function.]
...
double Extra_bddCountMinterms( DdManager * dd, DdNode * bFunc )
{
	assert( dd->size < 1024 );
	if ( s_Max == 0.0 )
		s_Max = Extra_Power2( dd->size );
	return extraBddCountMinterms( dd, bFunc );
} /* end of Extra_bddCountMinterms */
...
/**Function********************************************************************
  ...
  Synopsis    [Performs a recursive step of Extra_SymmPairsComputeFast.]
...
/**Function********************************************************************
  ...
  Synopsis    [Counts the number of minterms in the cofactor of a function.]
...
******************************************************************************/
double extraBddCountMintermsCofactor( DdManager * dd, DdNode * bFunc, DdNode * bVar )
{
	static _HashEntry_cof HHTable1[_TABLESIZE_COF];
	unsigned HKey;
	...
	if ( Cudd_IsComplement(bFunc) )
		return s_Max - extraBddCountMintermsCofactor( dd, Cudd_Not(bFunc), bVar );

	// now it is known that the function is not complemented
	if ( cuddIsConstant(bFunc) )
		return ((bFunc==b0)? 0: s_Max);

	// check cache
	HKey = hashKey2( bFunc, bVar, _TABLESIZE_COF );
	if ( HHTable1[HKey].bF == bFunc && HHTable1[HKey].bVar == bVar )
		return HHTable1[HKey].Res;
	else
	{
		double Res;
		DdNode * bVarR = Cudd_Regular(bVar);

		int LevelF = dd->perm[bFunc->index];
		int LevelV = dd->perm[bVarR->index];

		if ( LevelF < LevelV )
		{
			// Res = Res0/2 + Res1/2;
			Res = (extraBddCountMintermsCofactor( dd, cuddE(bFunc), bVar ) / 2) +
			      (extraBddCountMintermsCofactor( dd, cuddT(bFunc), bVar ) / 2);
		}
		else if ( LevelF == LevelV )
		{
			if ( bVarR != bVar )
				Res = extraBddCountMinterms( dd, cuddE(bFunc) );
			else
				Res = extraBddCountMinterms( dd, cuddT(bFunc) );
		}
		else // if if ( LevelF > LevelV )
			Res = extraBddCountMinterms( dd, bFunc );

		HHTable1[HKey].bF   = bFunc;
		HHTable1[HKey].bVar = bVar;
		HHTable1[HKey].Res  = Res;
		return Res;
	}
} /* end of extraBddCountMintermsCofactor */
...
/**Function********************************************************************
  ...
  SideEffects [May give incorrect result if the number of variables in the manager has changed.]

******************************************************************************/
double extraBddCountMinterms( DdManager * dd, DdNode * bFunc )
{
	static _HashEntry_mint HHTable2[_TABLESIZE_MINT];
	unsigned HKey;

	// normalize
	if ( Cudd_IsComplement(bFunc) )
		return s_Max - extraBddCountMinterms( dd, Cudd_Not(bFunc) );
	...
	// check cache
	HKey = hashKey1( bFunc, _TABLESIZE_MINT );
	if ( HHTable2[HKey].bF == bFunc )
		return HHTable2[HKey].Res;
	else
	{
		// Res = Res0/2 + Res1/2;
		double Res = (extraBddCountMinterms( dd, cuddE(bFunc) ) / 2) +
			         (extraBddCountMinterms( dd, cuddT(bFunc) ) / 2);

		HHTable2[HKey].bF  = bFunc;
		HHTable2[HKey].Res = Res;

		return Res;
	}
}	/* end of extraBddCountMinterms */
...
******************************************************************************/
unsigned cuddCacheLookup1Const( DdManager * table, DdNode*(*op)(DdManager*,DdNode*), DdNode * f )
{
	int posn;
	DdCache *en;
